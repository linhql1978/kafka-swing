...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="fix/messages/message|fix/components/component">
        ...
        <xsl:if test="$baseClass = 'quickfix.MessageComponent'">
            ...
            <xsl:apply-templates select="field|component" mode="component-field-numbers"/>
            };
            protected int[] getFields() { return componentFields; }
            private int[] componentGroups = {
            <xsl:apply-templates select="group" mode="component-field-numbers"/>
            };
        ...
        <xsl:choose>
            <xsl:when test="$orderedFields = 'true'">
                super(new int[] {
                <xsl:apply-templates select="field|component|group" mode="group-field-numbers"/>
                0 });
            </xsl:when>
            <xsl:otherwise>super();</xsl:otherwise>
        </xsl:choose>
        <xsl:if test="$baseClass = 'Message'">
            getHeader().setField(new <xsl:value-of select="$fieldPackage"/>.MsgType(MSGTYPE));
        </xsl:if>
        }
		...
		substring(@name, 2, string-length(@name)-1))"/>
                ...
                <xsl:value-of select="$fieldPackage"/>.<xsl:value-of select="concat(@name, ' ', $varname)"/>
            </xsl:for-each>
            ) {
            this();
            <xsl:for-each select="field[@required='Y']">
			...
			'abcdefghijklmnopqrstuvwxyz'),
		substring(@name, 2, string-length(@name)-1))"/>
                setField(<xsl:value-of select="$varname"/>);
            </xsl:for-each>
            }
        </xsl:if>
    ...
    <xsl:template mode="field-accessors" match="group">
        ...
        public static class
        <xsl:value-of select="@name"/> extends Group {

        static final long serialVersionUID = <xsl:value-of select="$serialVersionUID"/>;
        private static final int[] ORDER = {<xsl:apply-templates select="field|component|group"
                                                                 mode="group-field-numbers"/>0};

        public <xsl:value-of select="@name"/>() {
        super(<xsl:value-of select="/fix/fields/field[@name=$groupFieldName]/@number"/>,<xsl:apply-templates
            select="field|component|group" mode="group-delimeter"/>, ORDER);
        }
        <xsl:apply-templates select="field|component|group" mode="field-accessors"/>
        }
    </xsl:template>
    ...
    <xsl:template mode="group-delimeter" match="group//component">
        <xsl:if test="position() = 1">
            <xsl:variable name="name" select="@name"/>
            <xsl:apply-templates
                    select="/fix/components/component[@name=$name]/*[name(.)='field' or name(.)='group' or name(.)='component']"
    ...
    <xsl:template name="field-accessor-template">
        ...
        public <xsl:value-of select="$fieldPackage"/>.<xsl:value-of select="@name"/> get(<xsl:value-of
            select="$fieldPackage"/>.<xsl:value-of select="@name"/> value) throws FieldNotFound {
        getField(value);
        return value;
        }

        public <xsl:value-of select="$fieldPackage"/>.<xsl:value-of select="@name"/> get<xsl:value-of select="@name"/>()
    ...
    <xsl:template name="component-accessor-template">
        ...
        public
        <xsl:value-of select="$type"/> get(<xsl:value-of select="$type"/> component) throws FieldNotFound {
        getComponent(component);
        return component;
        }

        public
        <xsl:value-of select="$type"/> get<xsl:value-of select="@name"/>() throws FieldNotFound {
