...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:qf="http://www.quickfixj.org" version="2.0">
    ...
    <xsl:template match="fix/fields/field[@name=$fieldName]">
        ...
        public class
        <xsl:value-of select="@name"/> extends<xsl:call-template name="get-field-type"/>Field {
        ...
        data) {
        super(<xsl:value-of select="@number"/>, data<xsl:if test="@type='UTCTIMESTAMP' or @type='UTCTIMEONLY'">,
        true</xsl:if>);
        }
    ...
    <xsl:template name="values">
        <xsl:for-each select="value">
            ...
            <xsl:choose>
                <xsl:when test="../@type='STRING'">public static final String
                    <xsl:value-of select="$description"/> = "<xsl:value-of select="@enum"/>";
                </xsl:when>
                <xsl:when test="../@type='MULTIPLESTRINGVALUE'">public static final String
                    <xsl:value-of select="$description"/> = "<xsl:value-of select="@enum"/>";
                </xsl:when>
                <xsl:when test="../@type='MULTIPLEVALUESTRING'">public static final String
                    <xsl:value-of select="$description"/> = "<xsl:value-of select="@enum"/>";
                </xsl:when>
                <xsl:when test="../@type='BOOLEAN'">public static final boolean
                    <xsl:value-of select="$description"/> =<xsl:call-template name="y-or-n-to-bool"/>;
                </xsl:when>
                <xsl:when test="../@type='INT'">public static final int
                    <xsl:value-of select="$description"/> = <xsl:value-of select="@enum"/>;
                </xsl:when>
                <xsl:when test="../@type='NUMINGROUP'">public static final int
                    <xsl:value-of select="$description"/> = <xsl:value-of select="@enum"/>;
                </xsl:when>
                <xsl:when test="../@type='EXCHANGE'">public static final String
                    <xsl:value-of select="$description"/> = "<xsl:value-of select="@enum"/>";
                </xsl:when>
                <xsl:when test="../@type='MONTHYEAR'">public static final String
                    <xsl:value-of select="$description"/> = "<xsl:value-of select="@enum"/>";
                </xsl:when>
                <xsl:otherwise>public static final char
                    <xsl:value-of select="$description"/> = '<xsl:value-of select="@enum"/>';
                </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>
    ...
    <xsl:template name="version">fix<xsl:value-of select="//fix/@major"/><xsl:value-of select="//fix/@minor"/>
    </xsl:template>
