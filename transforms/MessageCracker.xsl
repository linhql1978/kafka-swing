...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="/">/* Generated Java Source File */
        ...
        */
        public void onMessage(quickfix.Message message, SessionID sessionID) throws FieldNotFound,
    ...
    <xsl:template name="virtual-functions">
        <xsl:for-each select="//fix/messages/message">
            ...
            * @throws IncorrectTagValue
            */
            public void onMessage(<xsl:value-of select="@name"/> message, SessionID sessionID) throws FieldNotFound,
            UnsupportedMessageType, IncorrectTagValue {<xsl:text/>
            <xsl:choose>
    ...
    <xsl:template name="switch-statement">
        ...
        /**
        * Cracker method for
        <xsl:value-of select="//fix/@major"/><xsl:value-of select="//fix/@minor"/> messages.
        *
        * @throws FieldNotFound
        * @throws UnsupportedMessageType
        * @throws IncorrectTagValue
        */
        public void crack<xsl:value-of select="//fix/@major"/><xsl:value-of select="//fix/@minor"/>(Message message,
        SessionID sessionID)
        throws UnsupportedMessageType, FieldNotFound, IncorrectTagValue {
